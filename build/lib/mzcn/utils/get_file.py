...
def get_file(
    ...
    cache_dir: typing.Union[Path, str] = mzcn.USER_DATA_DIR,
    verbose: int = 1
) -> str:
    """
    ...
    cache_dir `~/.mzcn/datasets`, placed in the cache_subdir `data`,
    and given the filename `fname`. The final location of a file
    `example.txt` would therefore be `~/.mzcn/datasets/data/example.txt`.
    ...
    :param cache_dir: Location to store cached files, when None it defaults to
        the [mzcn.USER_DATA_DIR](~/.mzcn/datasets).
    :param verbose: Verbosity mode, 0 (silent), 1 (verbose), 2 (semi-verbose)
    ...
    if not os.access(datadir_base, os.W_OK):
        datadir_base = os.path.join('/tmp', '.mzcn')
